"""StageProvider —— 把真 CodexRunner 包成真组件消费的 (cyc, pack)→files 阶段回调（M6 CP7.2）。
...
信封解析 + 逐产物 schema 校验 + artifact_parse 重试（§4.2.3）封成组件期望的 (cyc, pack)→files 签名，
从而真组件 + 真 Codex 端到端跑起来（run.py 装配，CP7.3）。

**provider 契约**（attack_stages 模块注释 / advancer reasoning_provider）——本类实现其中**产文件**的三个：
- idea(cyc, pack) → {"idea_set.json": …}
- plan(cyc, pack) → {"plan.json": …}
- reasoning(cyc, pack) → {"selection.json": …, "tree_ops.json"?: …, "answer.json"?: …}
judge（写 runner_call+DECISION，形态不同）+ bundle（由 plan 的 TARGET_SPEC 经 harness 驱动、非 Codex 调用）
不在本类——judge 随 §7.3 机制验收（attack 全链）落 CP7.4。

**职责边界**：本类只保证「产出**结构合法**（过 schema）的 files」；**语义**由组件把关——reasoning-only
轮的 create_root/add_children 必产、answer 时序、selection 可调度性等由 advancer/attack_stages 校验
（§4.2.3/§4.2.5）。故本类不判 answer_allowed 等轮型语义，只校验**在场**产物的 schema + 阶段必产文件在场。
...
**长操作零事务（§6.13）**：本类只跑 Codex 子进程 + 纯计算，绝不持写事务（组件在事务外调本类）。
"""
...
# 阶段 → 产物契约：required=阶段必产（缺即重试）、optional=在场才校验。判据同 driver._run_with_retry。
_STAGE_FILES = {
...
_CALL_NOTE = {
    ...
    "plan": "执行【计划任务】，产 plan.json（protocol + targets[TARGET_SPEC]）",
...
class StageProvider:
    ...
    def _validate_files(self, files: Dict[str, Any], spec: Dict[str, List[str]]) -> Optional[str]:
        """校验：required 全在场 + required/在场 optional 各过 schema。返回错误串（供反馈重试）或 None。"""
