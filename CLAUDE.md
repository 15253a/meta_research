...
审查：正确性 bug、契约破坏、并发 / 错误处理隐患、可回退性、可维护性 / 可读性（§10）。
...
[ ] 9. 写 build_log/NNNN（含步级验证结果）+ 更新 INDEX.md + 勾 ROADMAP.md（并同步其「当前位置」，§9）+ 刷新 implement_note.md → git commit 该记录（docs(build_log): …，引用检查点提交 hash，勿 amend）
...
- **位置指针双写（故意冗余）**：`implement_note.md` 的「位置」字段是**权威**（实时刷新）；`ROADMAP.md`「当前位置」是**兜底备份**——implement_note.md 覆盖式更新、有误写 / 丢失风险，ROADMAP 粗粒度、入 git，可供恢复。ROADMAP 的当前位置**只在记账时同步**（§5 步 9），检查点进行中它落后属正常；**两者不一致时以 implement_note.md 为准**。
...
## 10. 工程质量基线：可维护性与可读性（硬要求）

系统构建的一切制品——代码、prompt、skill、schema、接口文档——**必须可维护、可读**。这是验收标准的一部分，不是锦上添花；理由同 §9：这套系统会被**多个 session、多个模型**接力维护，写的时候就要假设"下一个读者零上下文、且不是你"。

- **为下一个接手者写**：命名把意图说清；模块小而单一职责、边界明确；宁可平直不要炫技——一眼看懂的三行 > clever 的一行。
- **注释只说"为什么"**（约束、取舍、陷阱），不复述代码"是什么"；契约 / 接口变更时同步更新相关文档（如 `INTERFACES.md`、schema 说明），不留漂移。
- **尺子**：零上下文的新 session（用便宜的模型验证，如 Sonnet 系列或 codex）读一个模块，短时间内能说清"它做什么、边界在哪、改哪里不会破坏别处"——做不到即可维护性缺陷。
- **评审执法**：检查点边界外审把可维护性 / 可读性列入审查重点（§2.1 模板、`bin/codex-review.sh`）——难读难维护按 [SHOULD] 起报，结构性失控可上 [BLOCKER]。
