...
import { isTransientHttpStatus } from "./provider-utils.mjs";
...
export function parseSemanticScholarRecord(record = {}) {
  const pdf = record?.openAccessPdf;
  if (!record?.isOpenAccess || !pdf?.url) return [];
  return [{
    source: "repository",
...
async function fetchJsonWithRetry(url, label, {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      if (!isTransientHttpStatus(response.status) || attempt === maxAttempts) break;
    } catch (error) {
  ...
  throw new Error(`${label} lookup failed: ${lastError || "request failed"}`);
}

export async function fetchUnpaywallRecord(doi, email, options = {}) {
...
export async function fetchSemanticScholarRecord(doi, options = {}) {
  if (!doi) return null;
  const paperId = encodeURIComponent(`DOI:${doi}`);
  const url = `https://api.semanticscholar.org/graph/v1/paper/${paperId}?fields=title,isOpenAccess,openAccessPdf`;
  return fetchJsonWithRetry(url, "Semantic Scholar", options);
}
