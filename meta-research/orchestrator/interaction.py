...
class InteractionIngest:
    ...
    def inbound(self, *, connector: str, raw_text: str, idempotency_key: str,
                ...
                cycle_id: Optional[str] = None, session_ref: Optional[str] = None,
        ...
        with self.daemon.transaction() as conn:
            ...
            return conn.execute(
                ...
                "raw_text,raw_hash,idempotency_key) VALUES (?,?,?,?,?,?,?,?,?)",
                (connector, conversation_id, session_ref, goal_id, goal_ver,
                 _cnum(cycle_id) if cycle_id else None,
                 raw_text, _sha(raw_text), idempotency_key)
            ).lastrowid

    def ack(self, *, message_id: int, reply_text: str, snapshot_cycle: Optional[str] = None,
            reply_role: str = "ack") -> int:
        """模板 ACK（带外回话；append-only）：写 interaction_reply(responder_kind='template')。
        **不写 decision**（§4.6.2 P1）；不改任何研究状态。真 codex 只读应答器 = M5。"""
        if reply_role not in ("ack", "final-template"):
            raise ValueError(f"interaction reply_role 非法: {reply_role!r}")
        with self.daemon.transaction() as conn:
            return conn.execute(
                "INSERT INTO interaction_reply(message_id,reply_ref,reply_hash,reply_text,snapshot_cycle,responder_kind) "
                "VALUES (?,?,?,?,?,'template')",
                (message_id, f"reply:{message_id}:{reply_role}", _sha(reply_text), reply_text,
