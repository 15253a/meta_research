...
class ResearchGraphInterface(HumanRequestOwnerInterface, Protocol):
    ...
    def query_target_measurement_asset_roles(
        self, evaluation_attempt_ref: str
...
class SQLiteResearchGraphReceiptVerifier:
    ...
    def _target_launch_authority(
        ...
        if target_ref in commit_by_target or any(
            ref not in commit_by_target for ref in target.dependency_refs
        ):
    ...
    def verify_bundle_dispatch_frontier(
        ...
        *,
        ...
        authoritative = tuple(
            ...
            if target.target_ref not in committed
...
class SQLiteResearchGraph(HumanRequestOwnerMixin):
    ...
    def query_target_measurement_asset_roles(
        ...
        """Read roles only from a current native Target measurement Attempt."""

        attempt = self.query_target_measurement_attempt(evaluation_attempt_ref)
        if attempt is None:
            return ()
        with self._database.read() as connection:
            rows = connection.execute(
