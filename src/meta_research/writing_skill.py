...
class CodexWritingSkillAdapter(CodexIdeaSkillAdapter):
    ...
    def generate_draft(self, request: WritingSkillRequest) -> WritingSkillDraft:
        ...
        prompt = (
            ...
            "v2 Snapshot 的 advancement.cycles 是完整研究历史；按每个 Cycle 的 "
            "question_ref 与 stage_history 中精确 epoch、commit_ref 汇总已接纳事实，"
            "包括 reasoning.result.outcome 中的科学结论、转换，以及 result 中的冻结证据。顶层 cycle/stages "
            "仅概览最新 Cycle，不能替代历史。skipped/exhausted 表示阶段处置，"
            "不能作为已完成的科学结论；没有全部四阶段也可以撰写阶段性报告。"
