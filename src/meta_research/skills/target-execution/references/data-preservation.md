...
最终交接前完成写入，并保持所选路径稳定至 Owner 接纳。持久保存由 Owner 接纳的精确版本与内容 receipt 证明；说明中的路径、hash 或单独 Dataset 登记都不能代替。暂存、未接纳材料留在工作区。原始数据、派生数据、文字、湿实验记录和辅助材料走相同保存路径。

Target 接纳后，Bundle／Reasoning 用精确 RM 版本登记值得复用的 Dataset 版本及用途；Target 的候选交接见[正式工作交接](formal-work.md)。以 `research_graph.datasets.derive` 关联源版本和派生版本，说明真实处理；多个来源分别记录。关系保存来源，不复制原件，也不自动构成科学结论。用 `research_graph.datasets.page` 的 `dataset_version_ref` 和 `direction` 读回来源或派生关系，并核对精确版本、当前用途引用及原件。
