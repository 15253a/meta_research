...
import { classifyWall, STATUS, isSuccess, isUserHandoff } from "./lib/status-codes.mjs";
...
import { fetchAndSaveFullText, saveFullTextResponse } from "./lib/direct-download.mjs";
...
export function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (k === "--no-institutional-access") a.noInstitutionalAccess = true;
  ...
  if (a.noInstitutionalAccess && (a.apiFallbackWeb || a.apiFallbackWebFor?.length)) {
    throw new Error("--no-institutional-access cannot be combined with institutional web fallback");
  }
  if (a.noInstitutionalAccess && a.topic) {
    throw new Error("--topic requires institutional discovery and cannot be combined with --no-institutional-access");
  }
  if (a.noInstitutionalAccess && a.si) {
    throw new Error("--si cannot be combined with --no-institutional-access");
  }
  parseSiChoice(a);
...
export function expandPdfCandidates(values = []) {
  const result = [];
  for (const value of values) {
    try {
      const url = new URL(value);
      if (/\/stamp\/stamp\.jsp$/i.test(url.pathname) && url.searchParams.get("arnumber")) {
        const pdf = new URL(url);
        pdf.pathname = "/stampPDF/getPDF.jsp";
        pdf.search = new URLSearchParams({
          tp: url.searchParams.get("tp") || "",
          arnumber: url.searchParams.get("arnumber"),
          ref: url.searchParams.get("ref") || "",
        }).toString();
        result.push(String(pdf));
      }
    } catch {
      continue;
    }
...
export function shouldPreserveBrowserTarget(status) {
...
export function publisherPdfHandoff(doi, url) {
  return {
    doi,
    status: STATUS.PUBLISHER_BLOCKED_WAITING_USER,
    url,
    reason: "Authenticated article page found, but no PDF candidate returned valid PDF bytes.",
    next_action: "Use the preserved tab for a manual download, or upload the full text for retry.",
  };
}
...
export async function downloadDoi(proxy, doi, outDir, wantSi, debug, {
  cleanBundle = false,
  bundleTitle = "",
  entryUrl = "",
  rewriteUrl = (value) => value,
} = {}) {
  const tab = (await newTab(proxy, entryUrl || "https://doi.org/" + doi)).targetId;
  let preserveTab = false;
  const handoff = (result) => {
    preserveTab = shouldPreserveBrowserTarget(result.status);
    return preserveTab ? { ...result, target_id: tab } : result;
  };
  try {
    ...
    if (wall) {
      ...
      if (verifyResult.passed) {
        ...
        if (!reWall) {
          ...
          return handoff({ doi, status: STATUS.VERIFICATION_AUTO_FAILED, url: info.url, reason: `auto-verify failed (${verifyResult.method || 'unknown'}), still: ${reWall.reason}` });
        }
      } else {
        return handoff({
          doi,
    ...
    for (let i = 0; i < 8; i++) {
      ...
      meta = JSON.parse(
        ...
        document.querySelectorAll('a').forEach(a=>{const h=a.href||'';const label=(a.innerText||a.textContent||'')+' '+(a.className||'');if(/\\/pdf|pdfdirect|pdfft|\\.pdf(\\?|$)|\\/doi\\/epdf/i.test(h)||/\\b(?:download\\s+)?pdf\\b/i.test(label))cand.push(h);});
    ...
    for (const pdfUrl of expandPdfCandidates(meta.cand)) {
      const got = await fetchToFile(proxy, tab, rewriteUrl(pdfUrl), pdfPath);
      if (got.ok) {
        const res = {
          doi,
    ...
    return handoff(publisherPdfHandoff(doi, meta.url));
  } finally {
    if (!preserveTab) await closeTab(proxy, tab);
  }
}

async function downloadDirectUrl(pdfUrl, outDir, title = "") {
  const saved = await fetchAndSaveFullText(pdfUrl, { outDir, title, source: pdfUrl });
  if (!saved.ok) {
    return { title, status: STATUS.PDF_FETCH_FAILED, url: pdfUrl, err: saved.reason || `HTTP ${saved.httpStatus}`, transferAttempts: saved.transferAttempts };
  }
  return {
    title,
...
async function runWebAccess(doi, article, args, context) {
  ...
  return downloadDoi(args.proxy, doi, args.out, args.si, args.debug, {
    ...
    entryUrl: context.institutionalDoiUrl(doi),
    rewriteUrl: context.institutionalUrl,
  });
}
...
export async function resolveExactTitleRequest(args, context, {
  downloadArxiv = downloadArxivTitle,
  findMetadata = findCrossrefByTitle,
} = {}) {
  let arxivAttempt = null;
  const priorAttempts = [];
  if (args.openAccess) {
    arxivAttempt = await downloadArxiv(args.title, args);
    if (arxivAttempt && isSuccess(arxivAttempt.status)) {
      return { dois: [], terminalResult: arxivAttempt, priorAttempts };
    }
    if (arxivAttempt) priorAttempts.push(attemptSummary(arxivAttempt, "arxiv"));
  }

  const metadata = await findMetadata(args.title, { mailto: context.contactEmail }).catch(() => null);
  if (metadata?.doi) {
    return { dois: [metadata.doi], terminalResult: null, priorAttempts };
  }

  if (!arxivAttempt) {
    arxivAttempt = await downloadArxiv(args.title, args);
    if (arxivAttempt) return { dois: [], terminalResult: arxivAttempt, priorAttempts };
  }
  return {
    dois: [],
    terminalResult: arxivAttempt || {
      title: args.title,
      status: args.openAccess ? STATUS.OA_NOT_FOUND : STATUS.METADATA_AMBIGUOUS,
      err: "no exact Crossref or arXiv title match",
...
async function main() {
  ...
  const context = {
    ...
    cnkiUrl,
    contactEmail,
    singleDoi: (args.dois || []).length === 1,
    institutionalUrl: (value) => institutionalUrlFromConfig(value, schoolConfig),
    institutionalDoiUrl: (doi) => institutionalUrlFromConfig(`https://doi.org/${doi}`, schoolConfig),
  };
  ...
  if (args.title && (isChineseLiterature({ title: args.title, language: args.language, sourceUrl: args.sourceUrl }) || args.route === "cnki") && !args.topic && !args.noInstitutionalAccess) {
  ...
  if (!dois.length && args.title && !args.topic) {
    const resolution = await resolveExactTitleRequest(args, context);
    if (resolution.terminalResult) {
      results.push(resolution.terminalResult);
      outputAndManifest(args, results, t0);
      return;
    }
    dois = resolution.dois;
    context.singleDoi = true;
    context.priorTitleAttempts = resolution.priorAttempts;
  }
  ...
  for (const doi of dois) {
    ...
    if (context.priorTitleAttempts?.length) {
      r.prior_oa_attempts = context.priorTitleAttempts;
    }
