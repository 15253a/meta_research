...
function usage() {
  ...
  node browser_pdf_downloader.mjs --target <targetId> --url <same-origin-pdf-url> --no-navigate --out <file.pdf>
...
export function parseArgs(argv) {
  ...
  if (!args.help) {
    ...
    if (args.noNavigate && (!args.target || !args.url)) {
      throw new Error("--no-navigate requires both --target and --url");
    }
  }
  return args;
}
...
async function main() {
  ...
  } else if (args.url && !args.noNavigate) {
