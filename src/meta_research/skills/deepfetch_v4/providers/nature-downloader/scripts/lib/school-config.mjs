...
export function institutionalUrlFromConfig(value, config) {
  const entryValue = config?.discovery?.resource_entry_url || config?.auth?.carsi_entry || "";
  let source;
  let entry;
  try {
  ...
  if (!["http:", "https:"].includes(source.protocol)) return value;
  const marker = entry.hostname.indexOf(".ivpn.");
  if (marker < 0) return value;
  const suffix = entry.hostname.slice(marker);
  if (source.hostname.endsWith(suffix)) return value;
  const encodedHost = source.hostname.replace(/\./g, "-") + (source.protocol === "https:" ? "-s" : "");
  const proxied = new URL(entry.origin);
  proxied.hostname = encodedHost + suffix;
  proxied.pathname = source.pathname;
  proxied.search = source.search;
  proxied.hash = source.hash;
