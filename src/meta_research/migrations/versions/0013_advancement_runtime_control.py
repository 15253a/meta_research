...
Revision ID: 0013_advancement_runtime_control
...
revision = "0013_advancement_runtime_control"
...
def _replace_stage_commits() -> None:
    ...
    try:
        ...
        op.create_table(
            ...
            sa.CheckConstraint("epoch >= 1"),
            sa.CheckConstraint(
                "(disposition = 'completed' AND request_ref IS NOT NULL AND "
                "run_ref IS NOT NULL AND "
                "outcome_ref IS NOT NULL AND outcome_kind IS NOT NULL AND "
                "run_completion_receipt_ref IS NOT NULL AND "
                "length(run_completion_receipt_hash) = 64 AND "
                "outcome_receipt_ref IS NOT NULL AND "
                "length(outcome_receipt_hash) = 64 AND basis_kind IS NULL AND "
                "basis_ref IS NULL AND basis_receipt_issuer IS NULL AND "
                "basis_receipt_kind IS NULL AND basis_receipt_subject_ref IS NULL "
                "AND basis_receipt_ref IS NULL AND "
                "basis_receipt_hash IS NULL) OR (disposition = 'skipped' AND "
                "request_ref IS NULL AND run_ref IS NULL AND outcome_ref IS NULL AND "
                ...
                "NULL AND outcome_receipt_hash IS NULL AND basis_kind IS NOT NULL "
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        "commits.stage = 'plan') THEN 'bundle' WHEN EXISTS (SELECT 1 FROM "
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            "'plan_review', 'deepfetch', 'experiment')"
        ),
        sa.CheckConstraint(
