...
def test_interrupted_sqlite_ddl_rolls_back_and_upgrade_can_restart(
    ...
    assert version == ("0009_human_collaboration",)
...
def test_interrupted_0003_ddl_rolls_back_the_whole_revision(
    ...
    with sqlite3.connect(database) as connection:
        assert connection.execute(
            "SELECT version_num FROM alembic_version"
        ).fetchone() == ("0009_human_collaboration",)
...
def test_0009_completed_initialization_becomes_recoverable_without_fake_auth(
    ...
    _downgrade_to_revision(database, "0008_quest_acquisition_session")
    with sqlite3.connect(database) as connection:
        assert connection.execute(
            "SELECT version_num FROM alembic_version"
...
def test_0009_first_reconcile_issues_one_marked_legacy_broad_authorization(
    ...
    _downgrade_to_revision(data_root.database, "0008_quest_acquisition_session")
    with sqlite3.connect(data_root.database) as connection:
        assert connection.execute(
