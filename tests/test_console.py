...
from orchestrator.console import Console, KeywordClassifier, sanitize
...
def test_inbound_idempotent_single_classification(env):
    ...
    c.confirm_directive(directive_id=r1["directive_id"], confirm_message_id=r1["message_id"])
...
def test_reject_by_user_no_decision(env):
    ...
    c.reject_directive(directive_id=r["directive_id"], reason="用户否掉润色稿")
...
def test_resume_supersedes_only_earlier_pending_pauses(env):
    ...
    c.confirm_directive(directive_id=rs["directive_id"], confirm_message_id=rs["message_id"])
...
def test_pending_directives_excludes_unconfirmed_hard(env):
    ...
    c.confirm_directive(directive_id=hard["directive_id"], confirm_message_id=hard["message_id"])
...
def test_abort_cycle_effect(env):
    ...
    c.confirm_directive(directive_id=r["directive_id"], confirm_message_id=r["message_id"])
    eff = c.consume_directive(directive_id=r["directive_id"], cycle_id="c1")
    assert eff["aborted_cycle"] == "c1"
    assert d.query_one("SELECT status FROM cycle WHERE id=1")[0] == "aborted"
