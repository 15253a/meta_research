...
def test_0014_backfills_existing_foreground_cycle_without_identity_rewrite(
    tmp_path: Path,
) -> None:
    ...
    _upgrade_to_revision(data_root.database, "0014_advancement_runtime_control")
...
def test_0014_rebuilds_the_current_stage_from_existing_commits(
    tmp_path: Path,
) -> None:
...
def test_0014_routes_an_existing_no_viable_idea_directly_to_reasoning(
    tmp_path: Path,
) -> None:
    data_root = prepare_data_root(tmp_path / "upgrade-no-viable-stage")
    _upgrade_to_revision(data_root.database, "0008_quest_acquisition_session")
    with sqlite3.connect(data_root.database) as connection:
        connection.execute("PRAGMA foreign_keys=ON")
        _seed_completed_idea_chain(connection)
        connection.execute(
            "UPDATE ae_stage_commits SET outcome_kind = 'no_viable_candidate' "
            "WHERE cycle_ref = 'cycle-idea' AND stage = 'idea'"
        )
    _upgrade_to_revision(data_root.database, "0012_experiment_measurement")
    _upgrade_to_revision(data_root.database, "0014_advancement_runtime_control")
    ...
    with sqlite3.connect(data_root.database) as connection:
        assert connection.execute(
            "SELECT stage FROM ae_cycles WHERE cycle_ref = 'cycle-idea'"
        ).fetchone() == ("reasoning",)
        assert connection.execute(
            "SELECT stage FROM ae_foreground_heads WHERE cycle_ref = 'cycle-idea'"
        ).fetchone() == ("reasoning",)
