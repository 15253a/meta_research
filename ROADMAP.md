...
- 进行中的步 / 检查点：**步⑪ CP11.3 状态与执行边界**。CP11.2a/CP11.2a.1 用户文件资产与嵌套树身份已落 `c077cd2`/`3c4c9b4`；
  ...
  CP11.3a 单实例 owner lease/heartbeat/lifecycle fencing 已落 `d79d3d0`；当前进入 CP11.3b 执行进程组与 owner-death 边界。
...
- 检查点（模型切，边走边补）：
  ...
  - [ ] CP11.3 状态与执行边界（子检查点全完成后勾选）：
    ...
    - [ ] CP11.3b 执行 owner-death 边界：统一普通 Codex、manifest/import/harness 的 process-group/cgroup supervisor，整组 timeout/退出清理与耐久 terminal receipt。
