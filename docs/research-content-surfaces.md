...
| `reasoning_evidence`（research_memory.reasoning_evidence.read） | Reasoning 冻结证据闭包的阶段上下文读取（只读，不接受新内容） | 阶段上下文 | 不承担发现（发现走五入口），不承担执行输入 |
...
- 文献：`stage_context.read` source=literature_records（按问题冻结修订分页读取记录）。
...
- Literature 尚无跨问题的独立分页发现入口；第一版经 question_history → 各问题文献修订读取。资产池增长后按实际瓶颈决定是否补 quest 级 literature 分页。
