...
- 更新：2026-07-08 ｜ 位置：步⑦（M6）CP7.5
- 检查点状态：自验通过（535 绿=532+3）→ 内审/外审中。改动=test_m6_longrun.py 新建（M6 长跑步级验证：
  ①守卫上界内广度填树无漂移[goal_ver 绑定/父指针有效/全 done/终 terminate]②整段 vs 分段换实例续跑终库
  逐字节一致[durable 交接]③τ 价值衰退自停+重启拒推进）。**关键裁量**：pure reasoning-only 受问题守卫
  上界约束（十余轮即填满树），「数百轮」需 attack 答问+前沿更新（阻于 plan 契约）——本套验**尺度不变**
  的不漂移/可恢复/自停三性质（docstring 明载）。
